...
2. **(One sample test, Raw values)** Some previous research found that the mean sepal length of setosas (a flower species) is 4.8 cm. A new investigation, however, show the following results in "iris\$Sepal.Length[1:50]". (display the results with: `head(iris,50)`) Conduct a hypothesis test at $.05$ significance level to show if there is sufficient evidence that the true mean of setosa sepal length is greater than 4.8 cm.
...
```{r}
setosa <- iris[iris$Species == "setosa",] # just take 1 species
